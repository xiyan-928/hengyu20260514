...
class FileService {
  ...
  Future<void> startNewSession() async {
    try {
      ...
      final newSessionDir = Directory('${baseDir.path}${Platform.pathSeparator}$newSessionDirName');
  ...
  Future<bool> saveSpectrumData(SpectrumData spectrumData, [String? customId, SpectrumFileFormat format = SpectrumFileFormat.spc]) async {
    try {
      await requestStoragePermission();
      
      // Use current system time from frontend instead of spectrum timestamp
      final currentTime = DateTime.now();
      final saveDir = await getSaveDirectory(currentTime); // Use Session Dir if active
      ...
      switch (format) {
        ...
        case SpectrumFileFormat.spc:
          ...
          success = await ApiSpcHandler.writeSpectrumToSpc(updatedSpectrumData, filePath);
          
          if (success) {
            // Store relative path (from Session Dir or Monthly Dir)
            // If in Session Dir, store relative to Base Dir? 
            // Or just store filename?
            // For now, we store relative to Base Dir if possible, or just dirName/fileName
            final dirName = saveDir.path.split(Platform.pathSeparator).last;
            latestSpcPath = '$dirName/$fileName';
      ...
      if (success) {
        print('Spectrum data saved to: ${saveDir.path}/$fileName');
      }
      return success;
    } catch (e) {
  ...
  Future<bool> saveSpectrumDataBothFormats(SpectrumData spectrumData, [String? customId]) async {
    final csvSuccess = await saveSpectrumData(spectrumData, customId, SpectrumFileFormat.csv);
    final spcSuccess = await saveSpectrumData(spectrumData, customId, SpectrumFileFormat.spc);
  ...
  Future<bool> saveSensorDataUnified(List<SensorData> sensorDataList, [DateTime? date]) async {
    try {
      await requestStoragePermission();
      
      final targetDate = date ?? DateTime.now();
      final saveDir = await getSaveDirectory(targetDate); // Use Session Dir if active
      ...
      final Set<String> sensorTypes = sensorDataList.map((data) => data.deviceType).toSet();
      ...
      if (fileExists) {
        ...
        final rows = existingContent.split('\n').where((row) => row.isNotEmpty).toList();
        
        if (rows.isNotEmpty) {
          // Parse header to get sensor types
          final headerRow = rows.first.split(',');
          final existingSensorTypes = headerRow.skip(1).toList(); // Skip 'Date' column
          ...
          for (int i = 1; i < rows.length; i++) {
            ...
            if (rowData.isNotEmpty) {
              ...
              for (int j = 1; j < rowData.length && j - 1 < existingSensorTypes.length; j++) {
  ...
  Future<bool> saveSensorData(List<SensorData> sensorDataList, [DateTime? date]) async {
    try {
      await requestStoragePermission();
      ...
      final currentTime = DateTime.now(); // Use current system time from frontend
      for (final sensorData in sensorDataList) {
        ...
        final updatedSensorData = SensorData(
          ...
          timestamp: currentTime, // Use current frontend time instead of backend timestamp
  ...
  Future<bool> saveSensorDataRow(
    ...
    try {
      ...
      if (_currentSessionPath == null) {
          if (_lastSaveDate != null) {
            ...
            if (lastStr != currentStr) {
              print('📅 Day changed from $lastStr to $currentStr, clearing directory cache');
      ...
      final activeSensorTypes = orderedSensorTypes.where((type) => 
        sensorDataMap.containsKey(type)).toList();
      ...
      print('保存传感器数据行: $timeStr -> ${activeSensorTypes.length} 个传感器, SPC: ${spcPath ?? "无"}');
      return true;
    } catch (e) {
      print('保存传感器数据行时出错: $e');
  ...
  Future<bool> saveAbsorbanceData(AbsorbanceData absorbanceData, [DateTime? date]) async {
    try {
      await requestStoragePermission();
