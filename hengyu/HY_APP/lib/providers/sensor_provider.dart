...
class SensorProvider with ChangeNotifier {
  ...
  static const int _maxHistoricalDataPoints = 50; // Reduced from 100 to 50 for better performance
  ...
  static const int _notificationThrottle = 3; // Update UI every 3rd collection cycle
  ...
  // Getters
  Map<String, SensorData> get currentSensorData => Map.unmodifiable(_currentSensorData);
  Map<String, List<SensorData>> get historicalSensorData => Map.unmodifiable(_historicalSensorData);
  List<String> get availableDeviceTypes => List.unmodifiable(_availableDeviceTypes);
  ...
  Future<bool> startCollection() async {
    if (_isCollecting) {
      if (kDebugMode) print('Collection already in progress, stopping current collection first');
      stopCollection();
    ...
    try {
      ...
      if (kDebugMode) print('🔥 SensorProvider setting up message stream listener: ${messageStream != null}');
      ...
      if (messageStream == null) {
        if (kDebugMode) print('🔥 SensorProvider ERROR: Unable to get message stream for sensor');
      }
      ...
      await _apiService.controlMonitor('start', interval: _collectionInterval / 1000.0);
      
      //------------------------------------
      await EdgeUploadCoordinator.acquire(_apiService, EdgeUploadChannel.sensor);
      
      _status = '传感器数据采集已启动';
      notifyListeners();
      
      if (kDebugMode) print('Sensor data collection started successfully with isolate');
      return true;
    } catch (e) {
      if (kDebugMode) print('Failed to start sensor collection with isolate: $e');
      _status = '启动采集失败: $e';
      _isCollecting = false;
      _isConnected = false;
      notifyListeners();
  ...
  void _handleIsolateMessage(Map<String, dynamic> message) {
    if (kDebugMode) print('🔥 SensorProvider received isolate message: $message');
    ...
    switch (type) {
      ...
      case 'error':
        ...
        break;
      case 'device_types':
        if (kDebugMode) print('🔥 Processing device_types message: ${message['deviceTypes']}');
        final deviceTypes = List<String>.from(message['deviceTypes'] ?? []);
        _availableDeviceTypes.clear();
        _availableDeviceTypes.addAll(deviceTypes);
        if (kDebugMode) print('🔥 Updated available device types: $_availableDeviceTypes');
        notifyListeners();
        break;
      default:
  ...
  void _processSensorDataFromIsolate(Map<String, dynamic> message) {
    try {
      ...
      for (final entry in data.entries) {
        ...
        if (kDebugMode) print('🔥 Updated sensor data for $deviceType: ${sensorData.value} ${sensorData.unit}');
      }
      ...
      if (kDebugMode) print('🔥 Current sensor data count: ${_currentSensorData.length}');
      if (kDebugMode) print('🔥 Historical data sensors: ${_historicalSensorData.keys.toList()}');
      
      // 立即更新UI以显示最新传感器数据
      _updateCounter++;
      if (kDebugMode) print('🔥 Notifying listeners immediately (update #$_updateCounter)');
      notifyListeners();
  ...
  void stopCollection() {
    ...
    //--------------------------------------
    unawaited(EdgeUploadCoordinator.release(_apiService, EdgeUploadChannel.sensor));
  ...
  void _restartCollection() async {
    if (_isCollecting) {
      print('Restarting collection with new interval: ${_collectionInterval}ms');
      stopCollection();
  ...
  Future<void> _collectAllSensorData() async {
    ...
    try {
      ...
      for (final deviceType in _availableDeviceTypes) {
        ...
        try {
          ...
          if (response.success && response.data != null) {
            ...
            successCount++;
            print('Successfully queried $deviceType: ${response.data} ${_getUnitForDeviceType(deviceType)}');
          } else {
      ...
      if (successCount > 0) {
        _status = '正在采集数据... ($successCount/${_availableDeviceTypes.length} 个传感器活跃)';
      } else {
        _status = '采集失败 - 未接收到数据';
      }
  ...
  Future<void> _collectAllSensorDataOptimized() async {
    // Prevent overlapping collection operations
    if (_isCollectionInProgress) {
      if (kDebugMode) print('Collection already in progress, skipping this cycle');
      return;
    }
    ...
    // Check if collection is still active before starting
    if (!_isCollecting) {
      if (kDebugMode) print('Collection stopped, aborting sensor data collection');
      return;
    }
    ...
    try {
      ...
      for (final deviceType in _availableDeviceTypes) {
        // Check if collection is still active before each sensor query
        if (!_isCollecting) {
          if (kDebugMode) print('Collection stopped during sensor queries, aborting');
          return;
        }
        ...
        try {
          ...
          // Check again after the async operation
          if (!_isCollecting) {
            if (kDebugMode) print('Collection stopped after querying $deviceType, aborting');
            return;
          }
          ...
          if (response.success && response.data != null) {
            ...
            successCount++;
            if (kDebugMode) print('Successfully queried $deviceType: ${response.data} ${_getUnitForDeviceType(deviceType)}');
          } else {
            if (kDebugMode) print('Failed to query $deviceType: ${response.message}');
            // Create a placeholder with error status for failed sensors
            if (!_currentSensorData.containsKey(deviceType)) {
              _currentSensorData[deviceType] = SensorData(
                deviceType: deviceType,
          ...
          // Check if collection is still active after error
          if (!_isCollecting) {
            if (kDebugMode) print('Collection stopped during error handling, aborting');
            return;
          }
          // Create a placeholder with error status for failed sensors
          if (!_currentSensorData.containsKey(deviceType)) {
      ...
      // Final check before updating status and saving data
      if (!_isCollecting) {
        if (kDebugMode) print('Collection stopped before final updates, aborting');
        return;
      }
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        if (_isAutoControlMode != isRunning) {
          if (kDebugMode) print('🔍 SensorProvider: Switching Auto Control Mode to $isRunning');
          _isAutoControlMode = isRunning;
          notifyListeners();
        }
        
        if (isActionActive && !_isCollecting && !_isAutoStarted) {
             if (kDebugMode) print('Auto-starting sensor collection due to backend action active.');
             ...
             if (_isAutoStarted) {
                 if (kDebugMode) print('Auto-stopping sensor collection due to backend action inactive.');
                 stopCollection();
